...
def prototype_initialize(model, data_loader, device, cfg):
    ...
    feat_estimator = prototype_dist_estimator(cfg, resume=None)
    model = model.module, model.eval()
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            data = scatter(data, device)[0]
            src_img, src_label = data['img'], data['gt_semantic_seg']

            src_feat = model.extract_bottlefeat(src_img)
            B, N, Hs, Ws = src_feat.shape

            # source mask: downsample the ground-truth label
            src_mask = (
                F.interpolate(
                    src_label.unsqueeze(0).float(), size=(Hs, Ws), mode='nearest'
                )
                .squeeze(0)
                .long()
            )
...
def main():
    ...
    data_cfg = cfg.data.train.source
    ...
    prototype_initialize(model, data_loader, cfg.gpu_ids, cfg.uda)
